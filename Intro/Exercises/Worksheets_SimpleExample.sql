...
-- What are the max sales prices for each of the three item subcategories (hot option, warm option, cold option)?
-- using subquery
select distinct item_subcategory, max_sale 
from (
    select item_subcategory,
    max(sale_price_usd) over(partition by item_subcategory) as max_sale
    from tasty_bytes_sample_data.raw_pos.menu
    -- where lower(item_subcategory) in ('hot option', 'warm option', 'cold option')
    )
order by 2 desc;
